...
public class SimpleModelGenerator {
	...
	private final CanStoreType canStoreType = new CanStoreType("pizza-carton-storage");
	private final StorableType storableType = new StorableType("pizza-carton", canStoreType);
	...
	public VRPSimulationModel generateSimpleModel(int seed) throws VRPArithmeticException, StorageException {
		...
		VRPSimulationModelParameters vrpSimulationModelParameters = new VRPSimulationModelParameters("Generated by SimpleModelGenerator with random seed " + seed + ".",
				"thomas.mayer@unibw.de");
		VRPSimulationModel model = new VRPSimulationModel(vrpSimulationModelParameters, structure, network);
		return model;
	}

	private Structure generateSimpleStructure(Network network, Random random) throws VRPArithmeticException, StorageException {
		INode home = network.getNodes().get(random.nextInt(network.getNodes().size() - 1));
		List<IDepot> depots = getDepots(1, 1, 10000, 10000, home);
		List<IVehicle> vehicles = getVehicles(1, 1, 40, home);
		List<IDriver> drivers = getDrivers(1, 1, home);
		List<ICustomer> customers = getCustomers(10, 1, 100, 0, network.getNodes(), random);
	...
	private List<ICustomer> getCustomers(int numberOfCustomers, int startIndex, double customerCapacity, int numberItemsInsideCustomer, List<INode> homes, Random random)
		...
		for (int i = startIndex; i < numberOfCustomers + startIndex; i++) {
			...
			VRPSimulationModelElementParameters vrpSimulationModelElementParameters = new VRPSimulationModelElementParameters("customer" + String.valueOf(i), 0);
			VRPSimulationModelStructureElementParameters vrpSimulationModelStructureElementParameters = new VRPSimulationModelStructureElementParameters(home);

			CanStoreParameters compartmentParameters = new CanStoreParameters(canStoreType, new Capacity(capacityUnit, customerCapacity), new LIFOLoadingPolicy(),
			...
			UncertainParameterContainer container = new UncertainParameterContainer(storableParameters, new DeterministicDistributionFunction(10.0),
					new DeterministicDistributionFunction(0.0), new DeterministicDistributionFunction(500.0), false);

			IStorableGenerator storableGenerator = new StorableGenerator(this.storableParameters);
			ICustomer customer = new StaticCustomerWithConsumption(vrpSimulationModelElementParameters, vrpSimulationModelStructureElementParameters,
					new UncertainParamters(container), storageManager, new DeterministicTimeFunction(10.0));
	...
	private List<IDepot> getDepots(int numberOfDepots, int startIndex, double depotCapacity, int numberItemsInsideDepot, INode home)
			throws VRPArithmeticException, StorageException {
		List<IDepot> depots = new ArrayList<>();
		for (int i = startIndex; i < numberOfDepots + startIndex; i++) {
			VRPSimulationModelElementParameters vrpSimulationModelElementParameters = new VRPSimulationModelElementParameters("depot" + String.valueOf(i), 0);
			VRPSimulationModelStructureElementParameters vrpSimulationModelStructureElementParameters = new VRPSimulationModelStructureElementParameters(home);

			CanStoreParameters compartmentParameters = new CanStoreParameters(canStoreType, new Capacity(capacityUnit, depotCapacity), new LIFOLoadingPolicy(),
					new StorableGenerator(this.storableParameters));
			ICanStore compartment = new Compartment(compartmentParameters);
			DefaultStorageManager storageManager = new DefaultStorageManager(new DefaultStorage(compartment));

			IStorableGenerator storableGenerator = new StorableGenerator(this.storableParameters);
			IDepot depot = new DefaultDepot(vrpSimulationModelElementParameters, vrpSimulationModelStructureElementParameters, new UncertainParamters(), storageManager,
					new DeterministicTimeFunction(20.0));
	...
	private Network generateSimpleRandomNetwork(int numberNodes, Random random) {
		...
		for (int i = 1; i <= numberNodes; i++) {
			VRPSimulationModelElementParameters vrpSimulationModelElementParameters = new VRPSimulationModelElementParameters("node" + String.valueOf(i), 0);
		...
		for (INode node1 : nodes) {
			...
			for (INode node2 : nodes) {
				VRPSimulationModelElementParameters vrpSimulationModelElementParameters = new VRPSimulationModelElementParameters(
						node1.getVRPSimulationModelElementParameters().getId() + "-" + node2.getVRPSimulationModelElementParameters().getId(), 0);
