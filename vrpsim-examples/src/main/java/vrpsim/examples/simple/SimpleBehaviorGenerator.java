...
public class SimpleBehaviorGenerator {
	...
	private ITour generateTour(List<String> customerIds, VRPSimulationModel model, IClock clock)
		...
		List<IActivity> activities = createActivities(customerIds, model, context, clock, model.getStructure().getStorableParameters().get(0));
		Tour tour = new Tour(context, activities);
		return tour;
	}

	private List<IActivity> createActivities(List<String> costomerIds, VRPSimulationModel model,
			TourContext context, IClock clock, StorableParameters storeableParameters) throws JobException, VRPArithmeticException {
		...
		for (String customerId : costomerIds) {
			...
			IActivity unloadAtCustomer = this.createStorableExchangeBetweenVehicleAndCustomer(
					context.getCurrentVehicle(), customer, model.getStructure().getDepots().get(0), storeableParameters);
			...
			if (storables - ((StorableExchangeJob) unloadAtCustomer.getJob()).getNumber() < 0) {
				...
				IActivity loadAtDepot = this.createStorableExchangeBetweenDepotAndVehicle(
						model.getStructure().getDepots().get(0), context.getCurrentVehicle(), storables, storeableParameters);
	...
	private StorableExchangeActivity createStorableExchangeBetweenDepotAndVehicle(IDepot depot, IVehicle vehicle, StorableParameters storeableParameters)
...
//		depot.getStorableGenerator().resetStorableGenerationCounter();
	...
	private StorableExchangeActivity createStorableExchangeBetweenVehicleAndCustomer(IVehicle vehicle,
			ICustomer customer, IDepot depot, StorableParameters storeableParameters) throws JobException, VRPArithmeticException {
