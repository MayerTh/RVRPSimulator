...
public class OPTICSAdapter {
	...
	public ClusterOrder executeOPTICS(double[][] data, DistanceMeasure distanceMeasure) {
		String[] labels = LabelUtil.generateLableList(data);
		return this.executeOPTICS(data, labels, distanceMeasure);
	}

	public ClusterOrder executeOPTICS(double[][] data, String[] labels, DistanceMeasure distanceMeasure) {
		...
		OPTICSHeap<NumberVector> optics = new OPTICSHeap<>(mappedDistanceFunction, epsilon, minPoints);
