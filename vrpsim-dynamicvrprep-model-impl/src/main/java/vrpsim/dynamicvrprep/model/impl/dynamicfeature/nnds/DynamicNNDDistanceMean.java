...
public class DynamicNNDDistanceMean extends ADynamicModelFeature {

	private static Logger logger = LoggerFactory.getLogger(DynamicNNDDistanceMean.class);
	...
	public Map<String, Double> calculateDynamicFeature(DynamicVRPREPModel model) {
		...
		try {
			tspFeatures = new TSPFeatures(TSPFeature.nnds_mean, false);
			result.put(TSPFeature.nnds_mean.toString(), tspFeatures.getFeature(model.getCoordinatesOfRequests(true))[0]);
			return result;
		} catch (InstantiationException | IllegalAccessException e) {
			e.printStackTrace();
			logger.error("Can not calculate feature {} due to {} {}.", this.getIdentifier(), e.getCause(), e.getMessage());
		}
		result.put(TSPFeature.nnds_mean.toString(), Double.NaN);
		return result;
	}
