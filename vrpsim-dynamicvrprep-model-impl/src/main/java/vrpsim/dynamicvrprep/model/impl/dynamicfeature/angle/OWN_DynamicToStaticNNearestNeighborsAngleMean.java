...
public class OWN_DynamicToStaticNNearestNeighborsAngleMean extends ADynamicModelFeature {

	private static Logger logger = LoggerFactory.getLogger(OWN_DynamicToStaticNNearestNeighborsAngleMean.class);
	private final int numberNeighbors;

	@Override
	public String getIdentifier() {
		return this.numberNeighbors + "NNAngleMean";
	}
	 ...
	 */
	public OWN_DynamicToStaticNNearestNeighborsAngleMean(int numberNeighbors) {
	...
	public Map<String, Double> calculateDynamicFeature(DynamicVRPREPModel model) {
		...
		for (double[] dynamicPoint : dynamicPoints) {
			...
			double v = StatUtils.mean(angles);
			values[index++] = v;
		}
