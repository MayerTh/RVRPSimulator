...
public class OWN_DynamicToStaticNNearestNeighborsDistanceMax extends ADynamicModelFeature {

	private static Logger logger = LoggerFactory.getLogger(OWN_DynamicToStaticNNearestNeighborsDistanceMax.class);
	private final int numberNeighbors;
	
	@Override
	public String getIdentifier() {
		return this.numberNeighbors + "NNDistanceMax";
	}

	public OWN_DynamicToStaticNNearestNeighborsDistanceMax(int numberNeighbors) {
	...
	public Map<String, Double> calculateDynamicFeature(DynamicVRPREPModel model) {
		...
		for (double[] dynamicPoint : dynamicPoints) {
			...
			double sumDistances = StatUtils.max(distances);
