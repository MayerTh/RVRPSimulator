...
public class OWN_DynamicToStaticNNearestNeighborsDistanceSum extends ADynamicModelFeature {

	private static Logger logger = LoggerFactory.getLogger(OWN_DynamicToStaticNNearestNeighborsDistanceSum.class);
	private final int numberNeighbors;
	
	@Override
	public String getIdentifier() {
		return this.numberNeighbors + "NNDistanceSum";
	}

	public OWN_DynamicToStaticNNearestNeighborsDistanceSum(int numberNeighbors) {
	...
	public Map<String, Double> calculateDynamicFeature(DynamicVRPREPModel model) {
		...
		for (double[] dynamicPoint : dynamicPoints) {
			...
			double sumDistances = StatUtils.sum(distances);
