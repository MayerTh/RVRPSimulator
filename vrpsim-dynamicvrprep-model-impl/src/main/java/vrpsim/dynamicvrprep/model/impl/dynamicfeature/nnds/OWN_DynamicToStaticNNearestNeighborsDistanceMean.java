...
public class OWN_DynamicToStaticNNearestNeighborsDistanceMean extends ADynamicModelFeature {

	private static Logger logger = LoggerFactory.getLogger(OWN_DynamicToStaticNNearestNeighborsDistanceMean.class);
	private final int numberNeighbors;
	
	@Override
	public String getIdentifier() {
		return this.numberNeighbors + "NNDistanceMean";
	}

	public OWN_DynamicToStaticNNearestNeighborsDistanceMean(int numberNeighbors) {
	...
	public Map<String, Double> calculateDynamicFeature(DynamicVRPREPModel model) {
		...
		for (double[] dynamicPoint : dynamicPoints) {
			...
			double sumDistances = StatUtils.mean(distances);
