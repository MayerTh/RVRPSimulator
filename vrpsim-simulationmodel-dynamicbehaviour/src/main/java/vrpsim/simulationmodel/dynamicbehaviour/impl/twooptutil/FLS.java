package vrpsim.simulationmodel.dynamicbehaviour.impl.twooptutil;
...
public final class FLS {
	...
	private static Logger logger = LoggerFactory.getLogger(FLS.class);

	/**
	 * 2-Opt a tour. removes 2 edges, then reconstructs a tour. in general (from
	 * http://en.wikipedia.org/wiki/2-opt): 1. take route[0] to route[i-1] and add
	 * them in order to new_route 2. take route[i] to route[k] and add them in
	 * reverse order to new_route 3. take route[k+1] to end and add them in order to
	 * new_route
	 *
	 * note that reverse is called with min(from,to)+1, max(from,to). in a 2-opt
	 * move, 2 edges are removed, leaving 2 disconnected sub-tours. either one of
	 * these subtours is then reversed and the 2 subtours reconnected in a different
	 * (shorter) way.
	 *
	 * Bentley, in his experiements on TSP heuristics paper notes that since either
	 * subtour can be reversed, it is best to reverse the shortest one, otherwise an
	 * arbitrary reversal will be N/2 array accesses.
	 *
	 * this implementation currently reverses whatever subtour does not wrap around
	 * -which could be the larger of the two.
	 */
	...
	/**
	 * cost of a 2-Opt. cost of replacing existing edges (ab), (cd) with new edges
	 * (ac) (bd). returns the delta of a 2-Opt move. a negative delta indicates that
	 ...
	 *
	 * this function is the main hotspot in the optimisation. it is not feasible to
	 * pre-compute a matrix (a lookup table) for a tour with N cities, since this
	 * will be O(N^2) and the most compact representation will be (N^2-N)/2.
	 *
	 * good optimisation: most of the time the algorithm is evaluating bad moves, in
	 * the obvious case where 2 edge exchanges would result in 2 longer edges, avoid
	 * 4 square root operations by comparing squares. this results in a 40% speed up
	 * in this code.
	 */
	private double moveCost(final Point a, final Point b, final Point c, final Point d) {
	 ...
	 */
	private void activate(final Point a, final Point b, final Point c, final Point d) {
		a.setActive(true);
		b.setActive(true);
		c.setActive(true);
		d.setActive(true);
	...
	/**
	 * try to find a move from the current city. given the current city, search for
	 * a 2-opt move that will result in an improvement to the tour length. the edge
	 * before the current city, (prevPoint,currenPoint) and after
	 * (currentPoint,nextPoint) are compared to all over edges (c,d), starting at
	 * (c=currentPoint+2, d=currentPoint+3) until an improvement is found.
	 */
	private double findMove(final int current, final Point currentPoint, final Point[] points, final int numCities) {
		...
		for (int i = wrap(current + 2, numCities), j = wrap(current + 3, numCities); j != current; i = j, j = wrap(j + 1, numCities)) {
	...
	public double optimise(final Point[] points) {
		...
		logger.debug("Tour cost before: {}", best);
		...
		logger.debug("Tour cost after: {}", best);
		return best;
	}
