...
public class Behaviour {
	...
	private final int numberOrignialTours;

	private BehaviourService behaviourService;

	public Behaviour(final List<ITour> tours) {
		this.tours = tours;
		this.numberOrignialTours = this.tours.size();
	}

	public void reset() {

		List<ITour> resetedTours = new ArrayList<>();
		for (int i = 0; i < this.numberOrignialTours; i++) {
			resetedTours.add(this.tours.get(i));
		}

		for (ITour tour : resetedTours) {
			tour.reset();
		}

		this.tours = resetedTours;
	}
	...
	/**
	 * Returns the {@link BehaviourService} which should be used to get information
	 * and services for the {@link Behaviour}.
	 * 
	 * @return
	 */
	...
	public void init() {
		this.tours.stream().forEach(e -> e.init());
	}
