...
public class Visualisation extends Application {
	...
	private InfoPaneController spc;

	public static void init(MainProgramm mainProgram, VRPSimulationModel simulationModel, Long simulationEndTime) {
	...
	public void start(Stage primaryStage) {
		...
		if (Visualisation.mainProgram == null || Visualisation.model == null || Visualisation.simulationEndTime == null) {

			String log = "vrpsim.visualization.Visualisation has to be initialized by "
					+ "calling void init(MainProgramm mainProgram, VRPSimulationModel " + "simulationModel, ITime simulationEndTime).";
	...
	public void initRootLayout() {
		try {
			...
			VBox infoBox = this.rootLayoutController.getInfoPane();
			AnchorPane orderPane = this.rootLayoutController.getOrderPane();
			this.spc = new InfoPaneController(infoBox, orderPane);
	...
	public void showModelView() {
		try {
			...
			for (Node n1 : rootLayout.getChildren()) {
				if (n1.getId() != null && n1.getId().equals("centerPane")) {
					for (Node n2 : ((SplitPane) n1).getItems()) {
						if (n2.getId() != null && n2.getId().equals("modelPane")) {
							((AnchorPane) n2).getChildren().add(modelView);
						}
					}
				}
			}
			...
			controller.init(model, spc, this.rootLayoutController.getOrientation());
