...
public class NetworkNodeVisualizationPopup extends VisualizationPopup {
	...
	private IVRPSimulationModelNetworkElement networkElement;

	public NetworkNodeVisualizationPopup(IVRPSimulationModelNetworkElement networkElement, double width, double hight,
			InfoPaneController controller, double closeImagaeBias) {
		super(controller, closeImagaeBias);
		this.networkElement = networkElement;
		if (networkElement instanceof INode) {
			INode node = (INode) networkElement;
			this.title = node.getVRPSimulationModelElementParameters().getId() + " at " + node.getLocation().toString();
			buildPopup(title, width, hight, true);
		}
	}

	public void update(List<IVRPSimulationModelStructureElement> elements, Double simulationTimeOfLastEventOccurence) {
		...
		for (IVRPSimulationModelStructureElement element : elements) {
			...
			if (element instanceof IVRPSimulationModelStructureElementWithStorage) {
				...
				for (CanStoreType type : sm.getCanStoreManager().getAllCanStoreTypes()) {
					...
					String currentCapa = sm.getCanStoreManager().getCurrentCapacity(type) >= Double.MAX_VALUE ? "~"
							: String.valueOf(sm.getCanStoreManager().getCurrentCapacity(type));
					String maxCapa = sm.getCanStoreManager().getCurrentCapacity(type)
							+ sm.getCanStoreManager().getFreeCapacity(type) >= Double.MAX_VALUE ? "~"
				...
				// for (UncertainParameterContainer container :
				// customer.getUncertainParameters().getParameter()) {
				//
				// String str = MessageFormat.format("UContainer-{0}:
				// start={1};number={2};type={6};edd={3};ldd={4};cyclic={5}", containerIndex++,
				// container.getNewRealizationFromStartDistributionFunction(),
				// container.getNewRealizationFromNumberDistributionFunction(),
				// container.getNewRealizationFromEarliestDueDateDistributionFunction(),
				// container.getNewRealizationFromLatestDueDateDistributionFunction(),
				// container.isCyclic(),
	...
	public IVRPSimulationModelNetworkElement getNetworkElement() {
